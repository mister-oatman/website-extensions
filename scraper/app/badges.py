...
def _logo_markup(platform: str, x: float, y: float, color: str) -> str:
    ...
    the count. ``x`` positions the glyph's visible ink, not its viewBox, so
    glyphs with internal whitespace (TikTok) line up like edge-to-edge ones
    (Instagram). An unknown platform yields no logo.

    Args:
        platform: The platform key (matched case-insensitively).
        x: Left edge of the logo's ink in the badge's coordinate space.
    ...
    if not path:
        return ""
    scale = LOGO_SIZE / 24
    ink_left = _LOGO_INK_X.get(platform.lower(), _FULL_INK_X)[0]
    transform = f"translate({x - ink_left * scale:.2f},{y:.2f}) scale({scale:.4f})"
...
def render_badge(
    ...
    if has_logo:
        ink_left, ink_right = _LOGO_INK_X.get(platform.lower(), _FULL_INK_X)
        width = round(text_w + LOGO_GAP + (ink_right - ink_left) * LOGO_SIZE / 24)
        height = LOGO_SIZE
